...
<?PHP 
    ...
    function uploadImgBase64 ($base64, $name){
        ...
        $path= $_SERVER['DOCUMENT_ROOT'].'/carnet-iutcm/Fotos/Foto-Estudiantes/'.$name;
	...
	$Foto= $_SERVER['DOCUMENT_ROOT'].'/carnet-iutcm/Fotos/Foto-Estudiantes/'. $_POST['Cedula'];
...
// Almacenar los datos
if($boton=="Guardar"){
	$sql="insert into regcarnetalum (Cedula, CodCarrera, CodSemes, CodTurno, NPago, FechaSol, idEstatus, Foto) values
	('$Cedula', '$CodCarrera', '$CodSemes', '$CodTurno', '$NPago', '$FechaSol', '$idEstatus','$Foto')";
	if(mysql_query($sql)){
			
			$idCarnet = mysql_insert_id();
			$sql="update alumcarnet set idCarnet = '$idCarnet' where Cedula='$Cedula'";
		
		mysql_query($sql);
		    // llamamos a la funcion uploadImgBase64( img_base64, Cedula.png) 
		    uploadImgBase64($_POST['imagen'], $_POST['Cedula'].'.png' );
		
				echo '<script src="bootstrap-3.3.7/js/jquery-3.2.1.min.js"></script>
					  <script>$(document).ready(function() {swal({ title:"Buen trabajo!!!",text:"Datos Guardados",type:"success"}).then(function() {window.location.href = "Generar-Carnet-Estudiantes.php";})});</script>';
...
if($boton=="Limpiar"){
echo "<script>window.location='Carnet-Estudiantes.php'</script>";
}
...
if($boton=="Modificar"){
	if($Cedula!=""){
            ...
            $sql="update alumcarnet set Cedula='$Cedula', idNacion='$idNacion', Nombres='$Nombres', Apellidos ='$Apellidos', idSexo='$idSexo', telefono='$telefono', correo='$correo' where Cedula='$Cedula'";
					mysql_query($sql);
						...
						if(mysql_query($sql)){
										...
										echo '<script>setTimeout(function(){swal({title:"Buen trabajo!!!",text:"Datos Modificados Correctamente",type:"success"}, function(isConfirm){ window.location = "Generar-Carnet-Estudiantes.php";});}, 100);</script>';
		...
		echo '<script>setTimeout(function(){swal({title:"OOPSS!",text:"Para poder Modificadar debe Realizar una busqueda",type:"error"}, 
													function(isConfirm){ window.location = "Generar-Carnet-Estudiantes";});}, 100);</script>';
	}
...
<nav class="navbar navbar-default">
	       	...
	       	<div class="container-fluid">
	       				<h2 class="text-center">Datos del Carnet Estudiantes</h2>
	       				<br>
												<form class="form-horizontal" action="" method="post" id="FormCarnEstu">
													<div class="row form-group ">
														...
														<div class="col-md-5">
																<div class="input-group">
									                    			<input type="text" class="form-control search-query" name="Cedula" placeholder="Cedula"  id="Cedula" value="<?php echo $Cedula;  ?>">
													...
													<div class="row form-group">
														...
														<div class="col-md-5">
															<input type="text" class="form-control" name="Nombres" placeholder="Nombres" value="<?php echo $Nombres;  ?>" id="Nombres" readonly>
															<span class="help-block"></span>
														</div>
													</div>
													<div class="row form-group">
														...
														<div class="col-md-5">
															<input type="text" class="form-control" name="Apellidos" placeholder="Apellidos" value="<?php echo $Apellidos;  ?>" id="Apellidos" readonly>
															<span class="help-block"></span>
														</div>		
													</div>
													<div class="row form-group">
														...
														<div class="col-md-5">
															<select class="form-control" name="CodCarrera" id="CodCarrera">
																<option value="" selected="" disabled="">Seleccione</option>
																<?php

																		$carrera ="Select * From carrera";
																		$result=mysql_query($carrera);
																		while($row = mysql_fetch_array($result)) {
																				echo "<option value='".$row['CodCarrera']."'>".$row['Carrera']."</option>";
																		}
																?>
															</select>
															<span class="help-block"></span>
													...
													<div class="row form-group">
														...
														<div class="col-md-5">
															<select class="form-control" name="CodSemes" id="CodSemes">
																<option value="" selected="" disabled="">Seleccione</option>
																<?php

																		$Semes ="Select * From semestre";
																		$result=mysql_query($Semes);
																		while($row = mysql_fetch_array($result)) {
																				echo "<option value='".$row['CodSemes']."'>".$row['Semes']."</option>";
																		}
																?>
															</select>
															<span class="help-block"></span>
														...
														<div class="col-md-5">
															<select class="form-control" name="CodTurno" id="CodTurno">
																...
																<?php
																		...
																		$result=mysql_query($Turno);
																		while($row = mysql_fetch_array($result)) {
																				echo "<option value='".$row['CodTurno']."'>".$row['Turno']."</option>";
																		}
																?>
															</select>
															<span class="help-block"></span>
													...
													<div class="row form-group">
														<div class="col-md-4 control-label">
															<label>N° de Pago</label>
														</div>
														<div class="col-md-5">
															<input type="text" class="form-control" name="NPago" placeholder="N° Pago" id="NPago">
															<span class="help-block"></span>
													</div>
													</div>
													<div class="row form-group">
...
<script type="text/javascript">
    	...
    	var FormCarnEstu = document.getElementById("FormCarnEstu");
...
function ValidaCodCarrera(){

    	CodCarrera = document.getElementById("CodCarrera").selectedIndex;
    	if( CodCarrera == null || CodCarrera == 0 ) {
    		$("#CodCarrera").parent().parent().attr("class", "form-group has-error has-feedback");
			$("#CodCarrera").parent().children("span").text("Debe seleccionar una carrera");
    		
  		return false;

		}

		$("#CodCarrera").parent().parent().attr("class", "form-group has-success has-feedback");
		return true;
    }
    function ValidaCodSemes(){

    	CodSemes = document.getElementById("CodSemes").selectedIndex;
    	if( CodSemes == null || CodSemes == 0 ) {
    		$("#CodSemes").parent().parent().attr("class", "form-group has-error has-feedback");
			$("#CodSemes").parent().children("span").text("Debe seleccionar un semestre");
    		
  		return false;

		}

		$("#CodSemes").parent().parent().attr("class", "form-group has-success has-feedback");
		return true;
    }
    function ValidaCodTurno(){

    	CodTurno = document.getElementById("CodTurno").selectedIndex;
    	if( CodTurno == null || CodTurno == 0 ) {
    		$("#CodTurno").parent().parent().attr("class", "form-group has-error has-feedback");
			$("#CodTurno").parent().children("span").text("Debe seleccionar un turno");
    		
  		return false;

		}

		$("#CodTurno").parent().parent().attr("class", "form-group has-success has-feedback");
		return true;
    }
    function ValidaNPago(){
			...
			if( NPago == null || NPago.length == 0 || /^\s+$/.test(NPago) ){
				$("#NPago").parent().parent().attr("class", "form-group has-error has-feedback");
				$("#NPago").parent().children("span").text("Este campo no puede estar vacio");
			  	return false;
		}
		else if  (!(/^\d*$/.test(NPago))){
				$("#NPago").parent().parent().attr("class", "form-group has-error has-feedback");
				$("#NPago").parent().children("span").text("Este campo no puede contener (a-zA-Z/!.*+-)");
			  	return false;
			}
			$("#NPago").parent().parent().attr("class", "form-group has-success has-feedback");
			return true;
    	}
	...
	function validacion(e){
			...
			if(ValidaCedula() && ValidaCodCarrera() && ValidaCodSemes() && ValidaCodTurno() && ValidaNPago()){
